...
#ifndef KOKKOS_XACC_PARALLEL_REDUCE_RANGE_HPP
#define KOKKOS_XACC_PARALLEL_REDUCE_RANGE_HPP

#include <XACC/Kokkos_XACC.hpp>
#include <XACC/Kokkos_XACC_Macros.hpp>
#include <XACC/Kokkos_XACC_FunctorAdapter.hpp>
#include <XACC/Kokkos_XACC_ScheduleType.hpp>
...
struct XACCParallelReduceHelper {
  XACCParallelReduceHelper(Functor const&, Reducer const&, Policy const&) {
...
class Kokkos::Impl::ParallelReduce<CombinedFunctorReducerType,
 ...
 public:
  ...
  void execute() const {
    ...
    Kokkos::Experimental::Impl::XACCParallelReduceHelper(
...
#define KOKKOS_IMPL_XACC_PARALLEL_REDUCE_DISPATCH_SCHEDULE(REDUCER,    \
  ...
  void XACCParallelReduce##REDUCER(Schedule<Static>, int chunk_size,   \
                                      IndexType begin, IndexType end,     \
                                      ValueType& aval,                    \
                                      Functor const& afunctor,            \
                                      int async_arg) {                    \
    /* FIXME_XACC FIXME_NVHPC workaround compiler bug (incorrect scope \
       analysis)                                                          \
    ...
    aval = val;                                                           \
                                                                          ...
                                                                          \
  template <class IndexType, class ValueType, class Functor>              \
  void XACCParallelReduce##REDUCER(Schedule<Dynamic>, int chunk_size,  \
                                      IndexType begin, IndexType end,     \
                                      ValueType& aval,                    \
                                      Functor const& afunctor,            \
                                      int async_arg) {                    \
...
#define KOKKOS_IMPL_XACC_PARALLEL_REDUCE_HELPER(REDUCER, OPERATOR)          \
  ...
  struct Kokkos::Experimental::Impl::XACCParallelReduceHelper<              \
      Functor, Kokkos::REDUCER<Scalar, Space>, Kokkos::RangePolicy<Traits...>, \
      true> {                                                                  \
    using Policy = RangePolicy<Traits...>;                                     \
    using ScheduleType =                                                       \
        Kokkos::Experimental::Impl::XACCScheduleType<Policy>;               \
    using Reducer   = REDUCER<Scalar, Space>;                                  \
    using ValueType = typename Reducer::value_type;                            \
                                                                               \
    XACCParallelReduceHelper(Functor const& functor,                        \
      ...
      int const chunk_size = policy.chunk_size();                              \
                                                                               \
      XACCParallelReduce##REDUCER(ScheduleType(), chunk_size, begin, end,   \
...
KOKKOS_IMPL_XACC_PARALLEL_REDUCE_HELPER(Sum, +);
KOKKOS_IMPL_XACC_PARALLEL_REDUCE_HELPER(Prod, *);
KOKKOS_IMPL_XACC_PARALLEL_REDUCE_HELPER(Min, min);
KOKKOS_IMPL_XACC_PARALLEL_REDUCE_HELPER(Max, max);
KOKKOS_IMPL_XACC_PARALLEL_REDUCE_HELPER(LAnd, &&);
KOKKOS_IMPL_XACC_PARALLEL_REDUCE_HELPER(LOr, ||);
KOKKOS_IMPL_XACC_PARALLEL_REDUCE_HELPER(BAnd, &);
KOKKOS_IMPL_XACC_PARALLEL_REDUCE_HELPER(BOr, |);

#undef KOKKOS_IMPL_XACC_PARALLEL_REDUCE_HELPER
