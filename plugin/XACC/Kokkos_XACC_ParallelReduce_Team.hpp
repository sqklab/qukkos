...
#ifndef KOKKOS_XACC_PARALLEL_REDUCE_TEAM_HPP
#define KOKKOS_XACC_PARALLEL_REDUCE_TEAM_HPP

#include <XACC/Kokkos_XACC_Team.hpp>
#include <XACC/Kokkos_XACC_FunctorAdapter.hpp>
#include <XACC/Kokkos_XACC_Macros.hpp>

#ifdef KOKKOS_ENABLE_XACC_COLLAPSE_HIERARCHICAL_CONSTRUCTS
#define KOKKOS_IMPL_XACC_LOOP_CLAUSE \
...
struct XACCParallelReduceTeamHelper {
  XACCParallelReduceTeamHelper(Functor const&, Reducer const&,
                                  Policy const&) {
...
class Kokkos::Impl::ParallelReduce<CombinedFunctorReducerType,
 ...
 private:
  using Policy =
      TeamPolicyInternal<Kokkos::Experimental::XACC, Properties...>;
 ...
 public:
  void execute() const {
    ...
    Kokkos::Experimental::Impl::XACCParallelReduceTeamHelper(
        Kokkos::Experimental::Impl::FunctorAdapter<
            FunctorType, Policy, KOKKOS_IMPL_XACC_LOOP_CLAUSE>(
    ...
    // XACC backend supports only built-in Reducer types; thus
    // reducer.final() below is a no-op.
    reducer.final(&val);
    // acc_wait(async_arg) in the below if-else statements is needed because the
    // above XACC compute kernel can be executed asynchronously and val is a
    // local host variable.
    if (m_result_ptr_on_device == false) {
      acc_wait(async_arg);
      *m_result_ptr = val;
  ...
  ParallelReduce(const CombinedFunctorReducerType& arg_functor_reducer,
      ...
      : m_functor_reducer(arg_functor_reducer),
        ...
        m_result_ptr_on_device(
            MemorySpaceAccess<Kokkos::Experimental::XACCSpace,
...
// FIXME_XACC: custom reduction is not implemented.
template <typename iType, class Lambda, typename ValueType, class JoinType>
KOKKOS_INLINE_FUNCTION void parallel_reduce(
    const Impl::TeamThreadRangeBoundariesStruct<iType, Impl::XACCTeamMember>&
        loop_boundaries,
...
KOKKOS_INLINE_FUNCTION void parallel_reduce(
    const Impl::ThreadVectorRangeBoundariesStruct<
        iType, Impl::XACCTeamMember>& loop_boundaries,
    const Lambda& lambda, const JoinType& join, ValueType& init_result) {
  static_assert(Kokkos::Impl::always_false<Lambda>::value,
                "custom reduction is not implemented");
}
...
parallel_reduce(const Impl::TeamThreadRangeBoundariesStruct<
  ...
  using functor_analysis_type = typename Impl::FunctorAnalysis<
      ...
      TeamPolicy<typename Impl::XACCTeamMember::execution_space>, Lambda,
      ValueType>;
...
parallel_reduce(const Impl::TeamThreadRangeBoundariesStruct<
  ...
  using functor_analysis_type = typename Impl::FunctorAnalysis<
      Impl::FunctorPatternInterface::REDUCE,
      TeamPolicy<typename Impl::XACCTeamMember::execution_space>,
...
template <typename iType, class Lambda, typename ValueType>
KOKKOS_INLINE_FUNCTION void parallel_reduce(
    const Impl::TeamVectorRangeBoundariesStruct<iType, Impl::XACCTeamMember>&
        loop_boundaries,
    const Lambda& lambda, ValueType& result) {
  using functor_analysis_type = typename Impl::FunctorAnalysis<
      Impl::FunctorPatternInterface::REDUCE,
...
#else /* #ifdef KOKKOS_ENABLE_XACC_COLLAPSE_HIERARCHICAL_CONSTRUCTS */
...
// FIXME_XACC: below implementation conforms to the XACC standard, but
...
#define KOKKOS_IMPL_XACC_PARALLEL_REDUCE_TEAM_THREAD(REDUCER, OPERATOR)   \
...
KOKKOS_IMPL_XACC_PARALLEL_REDUCE_TEAM_THREAD(Sum, +);
KOKKOS_IMPL_XACC_PARALLEL_REDUCE_TEAM_THREAD(Prod, *);
KOKKOS_IMPL_XACC_PARALLEL_REDUCE_TEAM_THREAD(Min, min);
KOKKOS_IMPL_XACC_PARALLEL_REDUCE_TEAM_THREAD(Max, max);
KOKKOS_IMPL_XACC_PARALLEL_REDUCE_TEAM_THREAD(LAnd, &&);
KOKKOS_IMPL_XACC_PARALLEL_REDUCE_TEAM_THREAD(LOr, ||);
KOKKOS_IMPL_XACC_PARALLEL_REDUCE_TEAM_THREAD(BAnd, &);
KOKKOS_IMPL_XACC_PARALLEL_REDUCE_TEAM_THREAD(BOr, |);
...
#define KOKKOS_IMPL_XACC_PARALLEL_REDUCE_THREAD_VECTOR(REDUCER, OPERATOR) \
...
KOKKOS_IMPL_XACC_PARALLEL_REDUCE_THREAD_VECTOR(Sum, +);
KOKKOS_IMPL_XACC_PARALLEL_REDUCE_THREAD_VECTOR(Prod, *);
KOKKOS_IMPL_XACC_PARALLEL_REDUCE_THREAD_VECTOR(Min, min);
KOKKOS_IMPL_XACC_PARALLEL_REDUCE_THREAD_VECTOR(Max, max);
KOKKOS_IMPL_XACC_PARALLEL_REDUCE_THREAD_VECTOR(LAnd, &&);
KOKKOS_IMPL_XACC_PARALLEL_REDUCE_THREAD_VECTOR(LOr, ||);
KOKKOS_IMPL_XACC_PARALLEL_REDUCE_THREAD_VECTOR(BAnd, &);
KOKKOS_IMPL_XACC_PARALLEL_REDUCE_THREAD_VECTOR(BOr, |);
...
#undef KOKKOS_IMPL_XACC_PARALLEL_REDUCE_TEAM_THREAD
#undef KOKKOS_IMPL_XACC_PARALLEL_REDUCE_THREAD_VECTOR

#endif /* #ifdef KOKKOS_ENABLE_XACC_COLLAPSE_HIERARCHICAL_CONSTRUCTS */

#define KOKKOS_IMPL_XACC_PARALLEL_REDUCE_DISPATCH_SCHEDULE(REDUCER,     \
                                                              OPERATOR)    \
  namespace Kokkos::Experimental::Impl {                                   \
  template <class Policy, class ValueType, class Functor>                  \
  void XACCParallelReduceTeam##REDUCER(Policy const policy,             \
                                          ValueType& aval,                 \
...
#define KOKKOS_IMPL_XACC_PARALLEL_REDUCE_TEAM_HELPER(REDUCER, OPERATOR) \
  KOKKOS_IMPL_XACC_PARALLEL_REDUCE_DISPATCH_SCHEDULE(REDUCER, OPERATOR) \
                                                                           \
  template <class Functor, class Scalar, class Space, class... Traits>     \
  struct Kokkos::Experimental::Impl::XACCParallelReduceTeamHelper<      \
    ...
    using ValueType = typename Reducer::value_type;                        \
                                                                           \
    XACCParallelReduceTeamHelper(Functor const& functor,                \
      ...
      int const async_arg = policy.space().acc_async_queue();              \
                                                                           \
      XACCParallelReduceTeam##REDUCER(policy, val, functor, async_arg); \
                                                                           \
      reducer.reference() = val;                                           \
    }                                                                      \
  }

KOKKOS_IMPL_XACC_PARALLEL_REDUCE_TEAM_HELPER(Sum, +);
KOKKOS_IMPL_XACC_PARALLEL_REDUCE_TEAM_HELPER(Prod, *);
KOKKOS_IMPL_XACC_PARALLEL_REDUCE_TEAM_HELPER(Min, min);
KOKKOS_IMPL_XACC_PARALLEL_REDUCE_TEAM_HELPER(Max, max);
KOKKOS_IMPL_XACC_PARALLEL_REDUCE_TEAM_HELPER(LAnd, &&);
KOKKOS_IMPL_XACC_PARALLEL_REDUCE_TEAM_HELPER(LOr, ||);
KOKKOS_IMPL_XACC_PARALLEL_REDUCE_TEAM_HELPER(BAnd, &);
KOKKOS_IMPL_XACC_PARALLEL_REDUCE_TEAM_HELPER(BOr, |);

#undef KOKKOS_IMPL_XACC_PARALLEL_REDUCE_TEAM_HELPER
#undef KOKKOS_IMPL_XACC_PARALLEL_REDUCE_DISPATCH_SCHEDULE
...
#endif /* #ifndef KOKKOS_XACC_PARALLEL_REDUCE_TEAM_HPP */
