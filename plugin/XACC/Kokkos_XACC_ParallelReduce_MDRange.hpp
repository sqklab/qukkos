...
#ifndef KOKKOS_XACC_PARALLEL_REDUCE_MDRANGE_HPP
#define KOKKOS_XACC_PARALLEL_REDUCE_MDRANGE_HPP

#include <XACC/Kokkos_XACC.hpp>
#include <XACC/Kokkos_XACC_Macros.hpp>
#include <XACC/Kokkos_XACC_FunctorAdapter.hpp>
#include <XACC/Kokkos_XACC_MDRangePolicy.hpp>
...
struct XACCParallelReduceMDRangeHelper {
  XACCParallelReduceMDRangeHelper(Functor const&, Reducer const&,
                                     Policy const&) {
...
class Kokkos::Impl::ParallelReduce<CombinedFunctorReducerType,
 ...
 public:
  ...
  void execute() const {
    ...
    Kokkos::Experimental::Impl::XACCParallelReduceMDRangeHelper(
...
#if defined(KOKKOS_ENABLE_XACC_COLLAPSE_MDRANGE_LOOPS)

#define KOKKOS_IMPL_XACC_PARALLEL_REDUCE_DISPATCH_ITERATE(REDUCER,       \
                                                             OPERATOR)      \
  namespace Kokkos::Experimental::Impl {                                    \
  template <class ValueType, class Functor>                                 \
  void XACCParallelReduce##REDUCER(XACCIterateLeft, ValueType& aval,  \
                                      Functor const& afunctor,              \
                                      XACCMDRangeBegin<2> const& begin,  \
                                      XACCMDRangeEnd<2> const& end,      \
                                      int async_arg) {                      \
    auto val = aval;                                                        \
    ...
    aval = val;                                                             \
                                                                            ...
                                                                            \
  template <class ValueType, class Functor>                                 \
  void XACCParallelReduce##REDUCER(XACCIterateRight, ValueType& aval, \
                                      Functor const& afunctor,              \
                                      XACCMDRangeBegin<2> const& begin,  \
                                      XACCMDRangeEnd<2> const& end,      \
                                      int async_arg) {                      \
  ...
  void XACCParallelReduce##REDUCER(XACCIterateLeft, ValueType& aval,  \
                                      Functor const& afunctor,              \
                                      XACCMDRangeBegin<3> const& begin,  \
                                      XACCMDRangeEnd<3> const& end,      \
                                      int async_arg) {                      \
    auto val = aval;                                                        \
    auto const functor(afunctor);                                           \
    auto begin2 = begin[2];                                                 \
  ...
  void XACCParallelReduce##REDUCER(XACCIterateLeft, ValueType& aval,  \
                                      Functor const& afunctor,              \
                                      XACCMDRangeBegin<4> const& begin,  \
                                      XACCMDRangeEnd<4> const& end,      \
                                      int async_arg) {                      \
    auto val = aval;                                                        \
    auto const functor(afunctor);                                           \
    auto begin3 = begin[3];                                                 \
  ...
  void XACCParallelReduce##REDUCER(XACCIterateLeft, ValueType& aval,  \
                                      Functor const& afunctor,              \
                                      XACCMDRangeBegin<5> const& begin,  \
                                      XACCMDRangeEnd<5> const& end,      \
                                      int async_arg) {                      \
    auto val = aval;                                                        \
    auto const functor(afunctor);                                           \
    auto begin4 = begin[4];                                                 \
  ...
  void XACCParallelReduce##REDUCER(XACCIterateLeft, ValueType& aval,  \
                                      Functor const& afunctor,              \
                                      XACCMDRangeBegin<6> const& begin,  \
                                      XACCMDRangeEnd<6> const& end,      \
                                      int async_arg) {                      \
    auto val = aval;                                                        \
    auto const functor(afunctor);                                           \
    auto begin5 = begin[5];                                                 \
...
#define KOKKOS_IMPL_XACC_PARALLEL_REDUCE_MDRANGE_HELPER(REDUCER, OPERATOR) \
  KOKKOS_IMPL_XACC_PARALLEL_REDUCE_DISPATCH_ITERATE(REDUCER, OPERATOR)     \
  template <class Functor, class Scalar, class Space, class... Traits>        \
  struct Kokkos::Experimental::Impl::XACCParallelReduceMDRangeHelper<      \
    ...
    using ValueType = typename Reducer::value_type;                           \
                                                                              \
    XACCParallelReduceMDRangeHelper(Functor const& functor,                \
...
KOKKOS_IMPL_XACC_PARALLEL_REDUCE_MDRANGE_HELPER(Sum, +);
KOKKOS_IMPL_XACC_PARALLEL_REDUCE_MDRANGE_HELPER(Prod, *);
KOKKOS_IMPL_XACC_PARALLEL_REDUCE_MDRANGE_HELPER(Min, min);
KOKKOS_IMPL_XACC_PARALLEL_REDUCE_MDRANGE_HELPER(Max, max);
KOKKOS_IMPL_XACC_PARALLEL_REDUCE_MDRANGE_HELPER(LAnd, &&);
KOKKOS_IMPL_XACC_PARALLEL_REDUCE_MDRANGE_HELPER(LOr, ||);
KOKKOS_IMPL_XACC_PARALLEL_REDUCE_MDRANGE_HELPER(BAnd, &);
KOKKOS_IMPL_XACC_PARALLEL_REDUCE_MDRANGE_HELPER(BOr, |);

#undef KOKKOS_IMPL_XACC_PARALLEL_REDUCE_MDRANGE_HELPER
#undef KOKKOS_IMPL_XACC_PARALLEL_REDUCE_DISPATCH_ITERATE
