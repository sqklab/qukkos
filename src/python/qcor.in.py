...
if '@QUKKOS_APPEND_PLUGIN_PATH@':
    sys.argv += ['__internal__add__plugin__path', '@QUKKOS_APPEND_PLUGIN_PATH@']

import xacc 

from _pyqukkos import *
import inspect, ast
...
# Hence, to be safe, we cache all the QJIT objects ever created until QUKKOS module is unloaded.
QJIT_OBJ_CACHE = []
@atexit.register
...
auto __internal__qukkos_pyjit_{}_gen_{}_unitary_matrix({}) {{
  auto py_src = R"#({})#";
  auto locals = py::dict();
  {}
...
class qjit(object):
    ...
    the QUKKOS QJIT infrastructure. Example usage:
    ...
    pythonic function body as an embedded domain specific language. The QUKKOS QJIT engine 
    takes this function string, and delegates to the QUKKOS Clang SyntaxHandler infrastructure, which 
    maps this function to a QUKKOS QuantumKernel sub-type, compiles to LLVM bitcode, caches that bitcode 
    ...
    qjit annotated functions can also be passed as general functors to other QUKKOS API calls like 
    ...
    def __convert__python__data__to__kernel__(self, python_data, *args, **kwargs):
        ...
        a python function adherent to the qukkos QJIT kernel model. Also return the function 
        body source string
        """
    ...
    def __init__(self, function, *args, **kwargs):
        ...
        self.allowed_type_cpp_map = {'<class \'_pyqukkos.qreg\'>': 'qreg',
                                     '<class \'_pyqukkos.qubit\'>': 'qubit',
                                     '<class \'float\'>': 'double', 'typing.List[float]': 'std::vector<double>',
                                     '<class \'int\'>': 'int', 'typing.List[int]': 'std::vector<int>',
                                     '<class \'_pyqukkos.Operator\'>': 'qukkos::Operator',
                                     'typing.List[typing.Tuple[int, int]]': 'PairList<int>',
                                     'typing.List[_pyqukkos.Operator]': 'std::vector<qukkos::Operator>'}
        self.__dict__.update(kwargs)

        # Create the qukkos just in time engine
        self._qjit = QJIT()
        self.extra_cpp_code = ''
        ...
        if 'with decompose' in fbody_src:
            ...
            for i, line_idx in enumerate(with_decomp_lines_idxs):
                ...
                if analyzer.depends_on:                 
                    ...
                    new_src += col_skip + ' '*4 + \
                        'auto [mat_data, mat_size] = __internal__qukkos_pyjit_{}_gen_{}_unitary_matrix({});\n'.format(self.kernel_name(),
        ...
        if not self.type_annotations or len(self.arg_names) != len(self.type_annotations):
            print('Error, you must provide type annotations for qukkos quantum kernels.')
            exit(1)
        ...
        # Create the qukkos quantum kernel function src for QJIT and the Clang syntax handler
    ...
    def translate(self, q: qreg, x: List[float]):
        ...
        if intersection == default_float_args:
            ...
            if len(ret_dict) != len(self.type_annotations):
                ...
                exit(1)
            return ret_dict
        elif [str(x) for x in self.type_annotations.values()] == ['<class \'_pyqukkos.qreg\'>', 'typing.List[float]']:
            ret_dict = {}
            for arg_name, _type in self.type_annotations.items():
                if str(_type) == '<class \'_pyqukkos.qreg\'>':
                    ret_dict[arg_name] = q
            ...
            if len(ret_dict) != len(self.type_annotations):
                ...
                exit(1)
            return ret_dict
        elif [str(x) for x in self.type_annotations.values()] == ['<class \'_pyqukkos.qreg\'>']:
            if len(x):
    ...
    def observe(self, observable, *args):
        ...
        # forwards the qreg to the qukkos::observe method so that users can get a handle to the qreg
        # which contains child buffer information (e.g. bitstrings/exp-val-z of each term).
        if (str(self.type_annotations[self.arg_names[0]]) == '<class \'_pyqukkos.qreg\'>' and args[0].size() == observable.nBits()):
            return internal_observe(program, observable, args[0])
        else:
            # Otherwise, just qukkos will use a temp. buffer and just return the expectation value.
    ...
    def construct_arg_dict(self, *args):
        ...
        for i, arg_name in enumerate(self.arg_names):
            ...
            if arg_type_str.startswith('typing.Callable'):
                ...
                # qjit is a pure-Python object, hence cannot be used by native QUKKOS.
...
class KernelBuilder(object):
    """
    The QUKKOS KernelBuilder is a high-level data structure that enables the 
    development of qukkos quantum kernels programmatically in Python. Example usage:
