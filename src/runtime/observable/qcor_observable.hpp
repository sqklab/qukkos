...
// This class implements the QUKKOS specification Operator concept.
...
class Operator
 ...
 private:
  class OperatorImpl;
  qukkos_pimpl<OperatorImpl> m_internal;
...
// Public QUKKOS Specification API for Operator creation (we retain the name Observable 
...
std::ostream &operator<<(std::ostream &os, qukkos::Operator const &m);
