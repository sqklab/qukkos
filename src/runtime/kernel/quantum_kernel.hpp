...
// The QuantumKernel represents the super-class of all qukkos
...
// class foo : public qukkos::QuantumKernel<class foo, qreg> {...};
...
class QuantumKernel {
 protected:
  ...
  std::shared_ptr<qukkos::CompositeInstruction> parent_kernel;
  ...
  // qukkos developers, not to be used by clients / programmers
 ...
 public:
  ...
  QuantumKernel(std::shared_ptr<qukkos::CompositeInstruction> _parent_kernel,
                Args... args)
  ...
  static double autograd(Operator &obs, std::vector<double> &dx, qreg q,
    ...
    std::function<std::shared_ptr<CompositeInstruction>(std::vector<double>)>
        kernel_eval = [q](std::vector<double> x) {
          auto tempKernel =
              qukkos::__internal__::create_composite("__temp__autograd__");
    ...
    auto gradiend_method = qukkos::__internal__::get_gradient_method(
        qukkos::__internal__::DEFAULT_GRADIENT_METHOD, kernel_eval, obs);
...
#define ONE_QUBIT_KERNEL_CTRL_ENABLER(CLASSNAME, QRTNAME)                 \
  class CLASSNAME : public OneQubitKernel<class CLASSNAME> {              \
   public:                                                                \
    ...
    CLASSNAME(std::shared_ptr<qukkos::CompositeInstruction> _parent_kernel, \
              qubit q)                                                    \
        : OneQubitKernel<CLASSNAME>(_parent_kernel, q) {                  \
      throw std::runtime_error("you cannot call this.");                  \
    }                                                                     \
    void operator()(qubit q) {                                            \
      parent_kernel = qukkos::__internal__::create_composite(               \
...
// The following is a first pass at enabling qukkos
  ...
  _qpu_lambda(std::string &&ff, std::string &&_capture_var_names,
    ...
    if (!capture_var_names.empty()) {
      ...
      for (auto [i, capture_name] :
           qukkos::enumerate(qukkos::split(capture_var_names, ','))) {
...
class KernelSignature {
 ...
 public:
  ...
  // CTor from a QUKKOS QuantumKernel instance:
  template <
      typename KernelType,
      std::enable_if_t<
...
void apply_control(std::shared_ptr<CompositeInstruction> parent_kernel,
  ...
  auto tempKernel = qukkos::__internal__::create_composite("temp_control");
  ...
  auto ctrlKernel = qukkos::__internal__::create_and_expand_ctrl_u(
  ...
  if (cached_is_compute_section) {
    for (int instId = 0; instId < ctrlKernel->nInstructions(); ++instId) {
      ctrlKernel->attachMetadata(instId,
                                 {{"__qukkos__compute__segment__", true}});
    }
  }
...
void apply_adjoint(std::shared_ptr<CompositeInstruction> parent_kernel,
                   KernelSignature<Args...> &kernelCallable, Args... args) {
  auto tempKernel = qukkos::__internal__::create_composite("temp_adjoint");
  kernelCallable(tempKernel, args...);
...
double observe(Operator &obs, KernelSignature<Args...> &kernelCallable,
               Args... args) {
  auto tempKernel = qukkos::__internal__::create_composite("temp_observe");
  kernelCallable(tempKernel, args...);
  auto instructions = tempKernel->getInstructions();
  // Assert that we don't have measurement
  if (!std::all_of(instructions.cbegin(), instructions.cend(),
  ...
  return qukkos::observe(tempKernel, observable, q);
}

template <typename... Args>
UnitaryMatrix as_unitary_matrix(KernelSignature<Args...> &kernelCallable,
                                Args... args) {
  auto tempKernel = qukkos::__internal__::create_composite("temp_as_unitary");
  kernelCallable(tempKernel, args...);
  auto instructions = tempKernel->getInstructions();
  // Assert that we don't have measurement
  if (!std::all_of(instructions.cbegin(), instructions.cend(),
...
template <typename... Args>
std::string openqasm(KernelSignature<Args...> &kernelCallable, Args... args) {
  auto tempKernel = qukkos::__internal__::create_composite("temp_as_openqasm");
  kernelCallable(tempKernel, args...);
...
void print_kernel(KernelSignature<Args...> &kernelCallable, std::ostream &os,
                  Args... args) {
  auto tempKernel = qukkos::__internal__::create_composite("temp_print");
  kernelCallable(tempKernel, args...);
  xacc::internal_compiler::execute_pass_manager(tempKernel);
...
std::size_t n_instructions(KernelSignature<Args...> &kernelCallable,
                           Args... args) {
  auto tempKernel = qukkos::__internal__::create_composite("temp_count_insts");
  kernelCallable(tempKernel, args...);
