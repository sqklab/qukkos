...
// TODO: implement QUKKOS native methods
std::vector<double> run_gradient_strategy(
    const std::vector<double> &x, double cost_val, const std::string &name,
    double step, qukkos::Operator &obs,
    std::function<
        std::shared_ptr<qukkos::CompositeInstruction>(std::vector<double>)>
        _kernel_eval) {
  auto observable =
  ...
  std::function<std::shared_ptr<xacc::CompositeInstruction>(
    ...
    // _kernel_eval produces a qukkos::CompositeInstruction ptr
...
class US_ABI_LOCAL FiniteDiffActivator : public BundleActivator {
 public:
  ...
  void Start(BundleContext context) {
    context.RegisterService<qukkos::KernelGradientService>(
        std::make_shared<qukkos::KernelForwardDifferenceGradient>());
    context.RegisterService<qukkos::KernelGradientService>(
        std::make_shared<qukkos::KernelBackwardDifferenceGradient>());
    context.RegisterService<qukkos::KernelGradientService>(
        std::make_shared<qukkos::KernelCentralDifferenceGradient>());
  }
