...
class VQEObjective : public ObjectiveFunction {
 public:
  ...
  double operator()(xacc::internal_compiler::qreg &qreg,
    ...
    if (!success) {
      xacc::error(
          "QUKKOS VQE Error - could not initialize internal xacc vqe algorithm.");
    }
    ...
    for (auto &child : tmp_child->getChildren()) {
      ...
      tmp.push_back(val);
      child->addExtraInfo("qukkos-params-energy", tmp);
      if (std::fabs(std_dev) > 1e-12) {
        child->addExtraInfo("qukkos-energy-stddev", std_dev);
      }
...
REGISTER_PLUGIN(qukkos::VQEObjective, qukkos::ObjectiveFunction)
