 ...
 *******************************************************************************/
#ifndef RUNTIME_QUKKOS_QRT_HPP_
#define RUNTIME_QUKKOS_QRT_HPP_
...
public:
  ...
  virtual void
  set_current_program(std::shared_ptr<qukkos::CompositeInstruction> p) = 0;
  virtual std::shared_ptr<qukkos::CompositeInstruction> get_current_program() = 0;
...
// qukkos quantum runtime library. The goal here is to provide
...
// exponential of i * theta * H, where H is an Observable pointer
void exp(qreg q, const double theta, qukkos::Operator &H);
...
// Some getters for the qukkos runtime library.
void set_current_program(std::shared_ptr<qukkos::CompositeInstruction> p);
...
// Disabled by default. Enabled by qukkos CLI option.
extern bool __print_opt_stats;

// User-customized passes to run
extern std::string __user_opt_passes;

// Placement strategy specified in the QUKKOS command line.
...
void execute_pass_manager(
    std::shared_ptr<qukkos::CompositeInstruction> optional_composite = nullptr);
std::string get_native_code(std::shared_ptr<qukkos::CompositeInstruction> program,
...
std::pair<bool, xacc::HeterogeneousMap> validate_backend_execution(
    std::shared_ptr<qukkos::CompositeInstruction> program = nullptr,
    xacc::HeterogeneousMap options = {});
} // namespace internal_compiler
} // namespace xacc
namespace qukkos {
