...
void execute_pass_manager(
    std::shared_ptr<qukkos::CompositeInstruction> optional_composite) {
  qukkos::internal::PassManager passManager(__opt_level, __qubit_map,
                                          __placement_name);
  ...
  for (const auto &user_pass : user_passes) {
    optData.emplace_back(
        qukkos::internal::PassManager::runPass(user_pass, kernelToExecute));
  }
...
void set_autograd_method(const std::string &method_name) {
  qukkos::__internal__::DEFAULT_GRADIENT_METHOD = method_name;
}

std::pair<bool, xacc::HeterogeneousMap>
validate_backend_execution(std::shared_ptr<qukkos::CompositeInstruction> program,
                           xacc::HeterogeneousMap options) {
  ...
  auto validator = xacc::getService<qukkos::BackendValidator>(DEFAULT_METHOD);
