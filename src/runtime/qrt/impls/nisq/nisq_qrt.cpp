...
namespace {
class NisqQubitAllocator : public qukkos::AncQubitAllocator {
 public:
...
class NISQ : public ::quantum::QuantumRuntime,
 ...
 protected:
  ...
  void one_qubit_inst(const std::string &name, const qubit &qidx,
    ...
    if (mark_as_compute) {
      inst->attachMetadata({{"__qukkos__compute__segment__", true}});
    }
 ...
 public:
  ...
  void exp(qreg q, const double theta, Operator &op) override {
    ...
    if (ptr_is_a<xacc::quantum::FermionOperator>(Hptr_input)) {
      ...
      xacc::error(
          "[qukkos::exp()] Error, cannot cast incoming Observable ptr to "
...
class US_ABI_LOCAL NisqQRTActivator : public BundleActivator {
 public:
  ...
  void Start(BundleContext context) {
    auto xt = std::make_shared<qukkos::NISQ>();
