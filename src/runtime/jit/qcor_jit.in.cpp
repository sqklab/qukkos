...
const std::pair<std::string, std::string> QJIT::run_syntax_handler(
  ...
  for (auto &arg : args_split) {
    ...
    // e.g. qukkos::_qpu_lambda<int const>& arg_0
  ...
  QUKKOSSyntaxHandler handler;
  qukkos::qpu_name = xacc::internal_compiler::get_qpu()->name();
  qukkos::shots = quantum::get_shots();
 ...
 public:
  ...
  Error addModule(std::unique_ptr<llvm::Module> M,
    ...
    MainJD.addGenerator(cantFail(DynamicLibrarySearchGenerator::Load(
        ...
        DL.getGlobalPrefix())));
    MainJD.addGenerator(cantFail(DynamicLibrarySearchGenerator::Load(
        "@CMAKE_INSTALL_PREFIX@/lib/libqukkos@CMAKE_SHARED_LIBRARY_SUFFIX@",
        DL.getGlobalPrefix())));
    MainJD.addGenerator(cantFail(DynamicLibrarySearchGenerator::Load(
...
void QJIT::jit_compile(std::unique_ptr<llvm::Module> m,
  ...
  if (!jit) {
    jit = cantFail(qukkos::LLVMJIT::Create(),
...
void QJIT::jit_compile(const std::string &code,
  ...
  if (cached_kernel_codes.count(hash)) {
    ...
    if (!jit) {
      ...
      jit = cantFail(qukkos::LLVMJIT::Create(std::move(ctx)));
    }

  } else {
    // We have not seen this code before, so we
    // need to map it to an LLVM Module
    act = qukkos::emit_llvm_ir(new_code, extra_headers);
  ...
  for (Function &f : *module) {
    ...
    if (demangled.find(kernel_name+"(") != std::string::npos && // has to be KERNEL_NAME(
        ...
        demangled.find("qukkos::QuantumKernel<"+kernel_name) == std::string::npos && // don't pick the QuantumKernel 
        demangled.find("__internal__compute_context_") == std::string::npos && // don't pick any of our internally generated compute functions
        demangled.find("std::shared_ptr<qukkos::CompositeInstruction>") != std::string::npos) { // must have composite inst arg
...
std::shared_ptr<qukkos::CompositeInstruction> QJIT::extract_composite_with_hetmap(
  ...
  void (*kernel_functor)(std::shared_ptr<qukkos::CompositeInstruction>,
      ...
      (void (*)(std::shared_ptr<qukkos::CompositeInstruction>,
