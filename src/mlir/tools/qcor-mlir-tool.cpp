...
#include "qukkos-mlir-helper.hpp"
...
cl::opt<std::string> mlir_specified_func_name(
    "internal-func-name", cl::desc("qukkos provided function name"));
...
int main(int argc, char **argv) {
  ...
  llvm::cl::ParseCommandLineOptions(argc, argv,
                                    "qukkos quantum assembly compiler\n");
  ...
  auto mlir_gen_result = qukkos::util::mlir_gen(inputFilename, !noEntryPoint,
  ...
  if (emitAction == Action::DumpMLIR) {
    if (qoptimizations) {
      ...
      if (mlir::failed(pm.run(module_op))) {
        std::cout << BOLD << RED
                  << "[qukkos-mlir-tool] Language-to-MLIR lowering failed.\n"
                  << CLEAR;
        return 1;
      }
    }
  ...
  if (mlir::failed(pm.run(module_op))) {
    std::cout << BOLD << RED
              << "[qukkos-mlir-tool] MLIR-to-LLVM_MLIR lowering failed.\n"
              << CLEAR;
    return 1;
  }
  ...
  if (!llvmModule) {
    std::cout << BOLD << RED
              << "[qukkos-mlir-tool] MLIR_LLVM-to-LLVM_IR lowering failed.\n"
              << CLEAR;
    return -1;
  }
