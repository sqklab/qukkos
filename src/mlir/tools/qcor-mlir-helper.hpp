  ...
  MlirGenerationResult(std::unique_ptr<mlir::MLIRContext> &&in_mlir_context,
    ...
    engine.registerHandler(
        [&, verbose_error](Diagnostic &diag) -> LogicalResult {
          if (verbose_error) {
            std::cout << "[qukkos-mlir] Dumping Module after error.\n";
            (*module_ref)->dump();
          }
          ...
          for (auto &n : diag.getNotes()) {
            ...
            n.print(os);
            os.flush();
            std::cout << BOLD << RED << "[qukkos-mlir] Reported Error: " << s << "\n"
                      << CLEAR;
          }
...
std::pair<SourceLanguage, std::unique_ptr<mlir::OwningModuleRef>> loadMLIR(
  ...
  std::shared_ptr<qukkos::QuantumMLIRGenerator> mlir_generator;
  if (src_language_type == SourceLanguage::QASM2) {
    mlir_generator = std::make_shared<qukkos::OpenQasmMLIRGenerator>(context);
  } else if (src_language_type == SourceLanguage::QASM3) {
    mlir_generator = std::make_shared<qukkos::OpenQasmV3MLIRGenerator>(context);
  } else {
