...
void Qasm3SyntaxHandler::GetReplacement(Preprocessor &PP, Declarator &D,
  ...
  pm.addPass(std::make_unique<qukkos::QuantumToLLVMLoweringPass>(true, unique_f_names));
  ...
  for (int i = 1; i < arg_type_strs.size(); i++) {
    ...
    if (arg_type_strs[i].find("qukkos::qreg") != std::string::npos) {
      type_name = "Array*";
  ...
  // e.g. map qukkos::qreg to Array* with q.raw_array();
  for (int i = 0; i < arg_type_strs.size(); i++) {
    if (arg_type_strs[i].find("qukkos::qreg") != std::string::npos) {
      ...
      sss << "auto __tmp_internal_vector_array_" << old_var_name
          << " = qukkos::qir::toArray(" << old_var_name << ");\n";
      std::replace(program_parameters.begin(), program_parameters.end(),
...
void Qasm3SyntaxHandler::AddToPredefines(llvm::raw_string_ostream &OS) {
  ...
  OS << "using qukkos::qubit;\n";
}
}  // namespace qukkos

static SyntaxHandlerRegistry::Add<qukkos::Qasm3SyntaxHandler> X(
