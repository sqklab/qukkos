...
cx q[0], q[1];
  ...
  auto llvm = qukkos::mlir_compile(src, "test", qukkos::OutputType::LLVMIR, true);
  std::cout << "LLVM:\n" << llvm << "\n";
  // No instrucions left
...
reset q[0];
  ...
  auto llvm =
      qukkos::mlir_compile(src, "test_kernel", qukkos::OutputType::LLVMIR, false);
  std::cout << "LLVM:\n" << llvm << "\n";
...
for i in [0:2*n + 1] {
 x qb;
    ...
    auto llvm = qukkos::mlir_compile(src, "test_kernel1",
...
d = measure q[0];
print("measure =", d);
QUKKOS_EXPECT_TRUE(d == 1);
)#";
  auto llvm =
      qukkos::mlir_compile(src, "test_kernel1", qukkos::OutputType::LLVMIR, false);
  std::cout << "LLVM:\n" << llvm << "\n";
  ...
  EXPECT_FALSE(qukkos::execute(src, "test_kernel1"));
}
...
QUKKOS_EXPECT_TRUE(c[3] == 1);
  ...
  auto llvm =
      qukkos::mlir_compile(qasm_code, "test_kernel1", qukkos::OutputType::LLVMIR, false);
  std::cout << "LLVM:\n" << llvm << "\n";
