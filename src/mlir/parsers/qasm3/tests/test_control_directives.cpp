...
QUKKOS_EXPECT_TRUE(iterate_value == 5);
QUKKOS_EXPECT_TRUE(hit_continue_value == 2);
...
QUKKOS_EXPECT_TRUE(iterate_value == 3);
print("made it out of the loop");)#";
  auto mlir = qukkos::mlir_compile(uint_index, "uint_index",
                                 qukkos::OutputType::MLIR, false);
  std::cout << mlir << "\n";
...
print(loop_count);
print(break_value);
QUKKOS_EXPECT_TRUE(sum_value == 4);
QUKKOS_EXPECT_TRUE(loop_count == 2);
QUKKOS_EXPECT_TRUE(break_value == 5);)#";
  auto mlir = qukkos::mlir_compile(uint_index, "uint_index",
                                 qukkos::OutputType::MLIR, false);
  std::cout << mlir << "\n";
...
QUKKOS_EXPECT_TRUE(i == 8);
QUKKOS_EXPECT_TRUE(j == 7);
)#";
  auto mlir = qukkos::mlir_compile(uint_index, "uint_index",
                                 qukkos::OutputType::MLIR, false);
  std::cout << mlir << "\n";
...
// Coverage for https://github.com/ORNL-QCI/qukkos/issues/211 as well
...
print("Result 1 =", val1);
QUKKOS_EXPECT_TRUE(val1 == arg_val);
// Call it with 20 -> activate the early return
int[64] val2 = generate_number(20);
print("Result 2 =", val2);
QUKKOS_EXPECT_TRUE(val2 == 3);
)#";
  auto mlir = qukkos::mlir_compile(uint_index, "uint_index",
                                 qukkos::OutputType::MLIR, false);
  std::cout << mlir << "\n";
...
QUKKOS_EXPECT_TRUE(c[0] == 1);
QUKKOS_EXPECT_TRUE(c[1] == 1);
QUKKOS_EXPECT_TRUE(c[2] == 0);
QUKKOS_EXPECT_TRUE(c[3] == 1);
)#";
  // Make sure we can compile this in FTQC.
  // i.e., usual if ...
  auto mlir =
      qukkos::mlir_compile(qasm_code, "iqpe", qukkos::OutputType::LLVMIR, false);
  std::cout << mlir << "\n";
  // Execute (FTQC + optimization): validate expected results: 1101
  EXPECT_FALSE(qukkos::execute(qasm_code, "iqpe"));
}
