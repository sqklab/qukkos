...
bit p1;
p1 = parity(b1);
QUKKOS_EXPECT_TRUE(p1 == 0);
print(p1);
)#";
  auto mlir = qukkos::mlir_compile(sub1, "check_parity_sub",
                                 qukkos::OutputType::MLIR, false);

  std::cout << mlir << "\n";
  EXPECT_FALSE(qukkos::execute(sub1, "check_parity_sub"));
}
...
print("Avg <X0X1> = ", exp_val);
  ...
  auto mlir = qukkos::mlir_compile(sub1, "check_deuteron",
                                 qukkos::OutputType::MLIR, false);

  std::cout << mlir << "\n";
  EXPECT_FALSE(qukkos::execute(sub1, "check_deuteron"));
}
...
measure cout[0] -> ans[8];)#";
  auto llvm_ir = qukkos::mlir_compile(sub1, "check_deuteron",
