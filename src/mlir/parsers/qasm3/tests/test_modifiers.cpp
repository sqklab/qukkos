...
pow(2) @ test q, qq;
    ...
    auto llvm = qukkos::mlir_compile(src, "pow_test", qukkos::OutputType::LLVMIR, true);
    std::cout << "LLVM:\n" << llvm << "\n";
    // Get the main kernel
...
pow(3) @ cx q, qq;
    ...
    auto llvm =
        qukkos::mlir_compile(src, "pow_test", qukkos::OutputType::LLVMIR, false, /* opt-level */0);
    std::cout << "LLVM:\n" << llvm << "\n";
    llvm = llvm.substr(llvm.find("@__internal_mlir_pow_test"));
    const auto last = llvm.find_first_of("}");
    llvm = llvm.substr(0, last + 1);
...
inv @ cx q[4], q[5];
    ...
    auto llvm = qukkos::mlir_compile(src, "inv_test", qukkos::OutputType::LLVMIR, false);
    std::cout << "LLVM:\n" << llvm << "\n";
...
inv @ s q[1];
    ...
    auto llvm =
        qukkos::mlir_compile(src, "inv_test", qukkos::OutputType::LLVMIR, false);
    std::cout << "LLVM:\n" << llvm << "\n";
    llvm = llvm.substr(llvm.find("@__internal_mlir_inv_test"));
    const auto last = llvm.find_first_of("}");
    llvm = llvm.substr(0, last + 1);
...
ctrl @ x q[0], q[1];
    ...
    auto llvm =
        qukkos::mlir_compile(src, "ctrl_test", qukkos::OutputType::LLVMIR, false);
    std::cout << "LLVM:\n" << llvm << "\n";
...
ctrl @ t q[0], q[1];
    ...
    auto llvm = qukkos::mlir_compile(src, "ctrl_test", qukkos::OutputType::LLVMIR,
                                   false, 0);
    std::cout << "LLVM:\n" << llvm << "\n";
    llvm = llvm.substr(llvm.find("@__internal_mlir_ctrl_test"));
    const auto last = llvm.find_first_of("}");
...
for val in [0:8] {
  ...
  QUKKOS_EXPECT_TRUE(ans[0] == expected[0]);
  QUKKOS_EXPECT_TRUE(ans[1] == expected[1]);
  if (test == 7) {
    QUKKOS_EXPECT_TRUE(ans[2] == 0);
  }
  if (test == 3) {
    QUKKOS_EXPECT_TRUE(ans[2] == 1);
  }
  if (ans[0]) {
    x q[0];
  }
  ...
  auto llvm =
      qukkos::mlir_compile(check_ccx, "ccx", qukkos::OutputType::LLVMIR, true);
  // Runt the test 
  // TODO: debug the JIT engine, failing to run this (okay with qukkos driver)
  // EXPECT_FALSE(qukkos::execute(check_ccx, "ccx"));
}
...
pow(2) @ ctrl @ t q, qq;
  ...
  auto llvm = qukkos::mlir_compile(check_nested, "nested",
                                 qukkos::OutputType::LLVMIR, false, 0);
  std::cout << "LLVM:\n" << llvm << "\n";
}
