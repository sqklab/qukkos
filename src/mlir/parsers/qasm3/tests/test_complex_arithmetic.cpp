...
int y = 2;
int test1 = 2^(j-y);
QUKKOS_EXPECT_TRUE(test1 == 8);
int test2 = j^(j-y);
QUKKOS_EXPECT_TRUE(test2 == 125);
)#";
  auto mlir = qukkos::mlir_compile(power_test, "power_test",
                                 qukkos::OutputType::MLIR, false);
  std::cout << mlir << "\n";
  EXPECT_FALSE(qukkos::execute(power_test, "power_test"));
}
...
print(c);
  ...
  auto mlir = qukkos::mlir_compile(qpe_test, "qpe_test",
                                 qukkos::OutputType::MLIR, false);
  std::cout << mlir << "\n";
  EXPECT_FALSE(qukkos::execute(qpe_test, "qpe_test"));
}
