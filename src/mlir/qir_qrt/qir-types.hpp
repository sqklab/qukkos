...
struct Array {
  ...
  Array(int64_t nbItems, int itemSizeInBytes = sizeof(int8_t *))
    ...
    qukkos::internal::AllocationTracker::get().onAllocate(this);
  };
  // Copy ctor:
  ...
  void add_ref() {
    m_refCount += 1;
    qukkos::internal::AllocationTracker::get().updateCount(this, m_refCount);
  }
  ...
  ~Array() { qukkos::internal::AllocationTracker::get().onDeallocate(this); }
...
struct TupleHeader {
  ...
  static TupleHeader *create(int size) {
    ...
    th->m_tupleSize = size;
    th->m_refCount = 1;
    qukkos::internal::AllocationTracker::get().onAllocate(th);
    return th;
  }
  ...
  bool release_ref() {
    ...
    if (m_refCount == 0) {
      ...
      qukkos::internal::AllocationTracker::get().onDeallocate(this);
      delete[] buffer;
      return true;
    }
...
struct Callable {
  ...
  Callable(qukkos::qsharp::IFunctor *in_functor) : m_functor(in_functor) {}
...
private:
  qukkos::qsharp::IFunctor *m_functor = nullptr;
...
namespace qsharp {
// A generic base class of qukkos function-like objects
