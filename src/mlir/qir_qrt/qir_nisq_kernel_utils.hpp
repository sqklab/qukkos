 ...
 *******************************************************************************/
#pragma once
// Helper macros to generate QUKKOS kernel wrapper for
...
#define qukkos_import_qsharp_kernel(...)                                         \
  GET_MACRO(_0, ##__VA_ARGS__, qukkos_import_qsharp_kernel_var,                  \
            qukkos_import_qsharp_kernel_no_var, unknown)                         \
  (__VA_ARGS__)

// Note: By convention, the Q# LLVM func name has the __body suffix:
// Strategy: define a __qpu__ function which call the LLVM func.
#define qukkos_import_qsharp_kernel_var(OPERATION_NAME, ...)                     \
...
#define qukkos_import_qsharp_kernel_no_var(OPERATION_NAME)                       \
...
// Usage:
// qukkos_import_qsharp_kernel(MyQsharpKernel, double, int);
// This will inject a QUKKOS kernel of signature:
// void MyQsharpKernel(qreg, double, int);
// which can be used in QUKKOS, e.g. supporting NISQ remote submit API
...
#define qukkos_import_qasm3_kernel(...)                                         \
  GET_MACRO(_0, ##__VA_ARGS__, qukkos_import_qasm3_kernel_var,                  \
            qukkos_import_qasm3_kernel_no_var, unknown)                         \
  (__VA_ARGS__)

// Note: By convention, the Q# LLVM func name has the __body suffix:
// Strategy: define a __qpu__ function which call the LLVM func.
#define qukkos_import_qasm3_kernel_var(OPERATION_NAME, ...)                     \
...
#define qukkos_import_qasm3_kernel_no_var(OPERATION_NAME)                       \
