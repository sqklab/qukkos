...
public:
  ...
  bool runOnModule(Module &M) override {
    ...
    for (Module::iterator f_iter = M.begin(), E = M.end(); f_iter != E;
      ...
      qukkos::SearchForCallsToThisFunction search(&F);
      search.visit(M);
      ...
      qukkos::FindFunctionVariableStoreInsts find_stores(var_names);
      find_stores.visit(F);
      ...
      qukkos::LLVM_IR_To_XACC to_xacc_visitor(new_to_old_qreg_names);
      ...
      qukkos::XACC_To_LLVM_IR visitor(F.getParent(), find_stores.stores,
                                    composite.get());
      visitor.visit(F);
