...
void QUKKOSSyntaxHandler::GetReplacement(Preprocessor &PP, Declarator &D,
                                       CachedTokens &Toks,
...
void QUKKOSSyntaxHandler::GetReplacement(
  ...
  for (int i = 0; i < program_arg_types.size(); ++i) {
    if (program_arg_types[i].find("KernelSignature") != std::string::npos ||
        program_arg_types[i].find("qukkos::_qpu_lambda") != std::string::npos) {
      qukkos::append_kernel(program_parameters[i], {}, {});
    }
  }
  ...
  // with XACC api calls
  qukkos::append_kernel(kernel_name, program_arg_types, program_parameters);
  ...
  auto new_src = qukkos::run_token_collector(PP, Toks, src_to_prepend,
  ...
  OS << "class " << kernel_name << " : public qukkos::QuantumKernel<class "
  ...
  OS << "friend class qukkos::QuantumKernel<class " << kernel_name << ", "
     << program_arg_types[0];
  for (int i = 1; i < program_arg_types.size(); i++) {
    OS << ", " << program_arg_types[i];
  }
  ...
  OS << "friend class qukkos::KernelSignature<"<< program_arg_types[0];
  for (int i = 1; i < program_arg_types.size(); i++) {
    OS << ", " << program_arg_types[i];
  }
  OS << ">;\n";
  ...
  OS << "if (!parent_kernel) {\n";
  OS << "parent_kernel = "
        "qukkos::__internal__::create_composite(kernel_name);\n";
  OS << "}\n";
  ...
  OS << kernel_name << "(std::shared_ptr<qukkos::CompositeInstruction> _parent, "
  ...
  auto s = OS.str();
  qukkos::info("[qukkos syntax-handler] Rewriting " + kernel_name + " to\n\n" + s);
}

void QUKKOSSyntaxHandler::AddToPredefines(llvm::raw_string_ostream &OS) {
  if (__internal__developer__flags__::add_predefines) {
    OS << "#include \"qukkos.hpp\"\n";
    OS << "using namespace qukkos;\n";
...
class QUKKOSArgs : public PluginASTAction {
 public:
...
static SyntaxHandlerRegistry::Add<qukkos::QUKKOSSyntaxHandler> X(
    "qukkos", "qukkos quantum kernel syntax handler");

static FrontendPluginRegistry::Add<qukkos::QUKKOSArgs> XX("qukkos-args", "");
