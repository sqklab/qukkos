...
std::string run_token_collector(
  ...
  if (!xacc::isInitialized()) {
    ...
    // if so add qukkos plugins path to plugin search path
    if (std::string(XACC_ROOT) != std::string(QUKKOS_INSTALL_DIR)) {
      xacc::addPluginSearchPath(std::string(QUKKOS_INSTALL_DIR) +
                                std::string("/plugins"));
    }
    ...
    std::string qukkos_config_xacc_root(XACC_ROOT);
  ...
  for (std::size_t i = 0; i < Toks.size(); i++) {
    // FIXME if using and using qukkos::LANG;
    ...
    if (PP.getSpelling(Toks[i]) == "compute") {
      ...
      // Take them, pass them to qukkos syntax handler, to get back
      ...
      auto src_code = ::__internal__::qukkos::construct_kernel_subtype(
