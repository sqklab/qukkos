...
TEST(TimeSeriesQpeTester, checkPronyMethod) {
  ...
  auto result = qukkos::QuaSiMo::pronyFit(y_vec);
  size_t idx = 0;
...
TEST(TimeSeriesQpeTester, checkSimple) {
  ...
  for (const auto &angle : angles) {
    ...
    const auto expVal =
        evaluator->evaluate(std::make_shared<qukkos::CompositeInstruction>(kernel));
...
TEST(TimeSeriesQpeTester, checkMultipleTerms) {
  ...
  for (const auto &angle : angles) {
    ...
    const auto expVal = evaluator->evaluate(std::make_shared<qukkos::CompositeInstruction>(kernel));
    const auto refResult = refEvaluator->evaluate(std::make_shared<qukkos::CompositeInstruction>(kernel));
    std::cout << "Angle = " << angle << ": Exp val = " << expVal << " vs "
              << refResult << "\n";
