...
ModelFactory::createModel(ModelType type, const HeterogeneousMap &params) {
  if (type == ModelType::Heisenberg) {
    ...
    qukkos::Operator *model_observable = nullptr;
    // Get QUKKOS Pauli Op first (if any)
    if (params.keyExists<qukkos::Operator>("observable")) {
      static auto cached_pauli = params.get<qukkos::Operator>("observable");
      model_observable = &cached_pauli;
    }
    // Generic qukkos::Operator
    else if (params.pointerLikeExists<qukkos::Operator>("observable")) {
      model_observable = params.getPointerLike<qukkos::Operator>("observable");
    }
    else {
      ...
      if (!xacc::container::contains(SUPPORTED_OBS, observable_type)) {
        qukkos::error("Unknown observable type: " + observable_type);
      }

      auto observable = new qukkos::Operator("pauli", "0.0");
    ...
    QuaSiMo::TdObservable H = [&](double t) {
      qukkos::Operator tdOp("pauli", "0.0");
    ...
    if (std::find(hs_model->initial_spins.begin(), hs_model->initial_spins.end(),
      ...
      auto initialSpinPrep = qukkos::__internal__::create_composite("InitialSpin");
      auto provider = qukkos::__internal__::get_provider();
    ...
    qukkos::error("Unknown model type.");
