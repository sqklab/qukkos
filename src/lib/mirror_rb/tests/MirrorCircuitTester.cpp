...
TEST(MirrorCircuitTester, checkU3Inverse) {
  ...
  for (int i = 0; i < NUM_TESTS; ++i) {
    ...
    auto [mirror_cir, expected_result] = qukkos::MirrorCircuitValidator::createMirrorCircuit(
        std::make_shared<qukkos::CompositeInstruction>(circuit));
...
TEST(MirrorCircuitTester, checkNoise) {
  ...
  auto validator = xacc::getService<qukkos::BackendValidator>("mirror-rb");
  {
    auto [success, data] = validator->validate(
        accelerator, std::make_shared<qukkos::CompositeInstruction>(circuit));
    EXPECT_TRUE(success);
  }
  {
