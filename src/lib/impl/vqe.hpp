...
__qpu__ void estimate_energy(qreg q, const std::function<void(qreg)> &statePrep,
                             qukkos::Operator observable, int nSamples,
                             double &out_energy) {
  ...
  for (auto pauliInst : observable.getNonIdentitySubTerms()) {
    ...
    std::vector<qukkos::Operator> ops;
    for (auto [i, x_val] : enumerate(xv)) {
      auto z_val = zv[i];
      if (x_val == z_val) {
        // Y(q[i]);
        ops.emplace_back(qukkos::Y(i));
      } else if (x_val == 0) {
        // Z(q[i]);
        ops.emplace_back(qukkos::Z(i));

      } else {
        // X(q[i]);
        ops.emplace_back(qukkos::X(i));
      }
    }
    if (!ops.empty()) {
