...
int main(int argc, char** argv) {
  ...
  qukkos_expect(q.counts()["0"] > 400);
  qukkos_expect(q.counts()["1"] > 400);
  qukkos_expect(q.counts()["0"] + q.counts()["1"] == 1024);
  ...
  r.print();
  // H - H == I
  qukkos_expect(r.counts()["0"] == 1024);
  // b still uses n = 1 (capture by value...)
  rb.print();
  qukkos_expect(rb.counts().size() == 2);
  qukkos_expect(rb.counts()["0"] > 400);
  qukkos_expect(rb.counts()["1"] > 400);
  qukkos_expect(rb.counts()["0"] + rb.counts()["1"] == 1024);
  ...
  for (size_t i = 0; i < angles.size(); ++i) {
    ...
    qukkos_expect(std::abs(expectedResults[i] - exp) < 0.1);
  }
  ...
  qukkos_expect(test_val == 2);

  add_two(buffer_test, test_val);
  qukkos_expect(test_val == 4);
  ...
  qukkos_expect(test_val_const == 12);
  ...
  std::cout << "Sum: " << check << "\n";
  qukkos_expect(std::abs(check - 6.0) < 1e-12);
  check = 0.0;
  // Inline construction
  vector_sum(qalloc(1), std::vector<double>{2.0, 4.0, 6.0}, check);
  std::cout << "Sum: " << check << "\n";
  qukkos_expect(std::abs(check - 12.0) < 1e-12);
}
