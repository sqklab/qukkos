...
# We need to translate OpenFermion ops into qukkos Operators to use with kernels...
exp_args_qukkos = [createOperator('fermion', fop) for fop in exp_args_openfermion]
...
def obj(x : List[float]):
    e_at_x = ansatz.observe(H, qalloc(3), x, exp_args_qukkos)
    print(x,e_at_x)
    return e_at_x
