...
qukkos_include_qsharp(QUKKOS__Deuteron__Interop, double, double, int64_t);
...
int main() {
  ...
  const auto angles = qukkos::linspace(-M_PI, M_PI, 20);
  for (size_t i = 0; i < angles.size(); ++i) {

    const double angle = angles[i];
    const double exp_val_xx = QUKKOS__Deuteron__Interop(angle, 1024);
    std::cout << "<X0X1>(" << angle << ") = " << exp_val_xx
              << " vs. expected = " << expectedResults[i] << "\n";
    qukkos_expect(std::abs(exp_val_xx - expectedResults[i]) < 0.1);
  }
  return 0;
}