...
// Compile:
// qukkos -qdk-version 0.17.2106148041-alpha qft.qs qft_driver.cpp -shots 1024 -print-final-submission
using QPEOracleSignature = KernelSignature<qubit>;
qukkos_import_qsharp_kernel(QUKKOS__IQFT);
...
__qpu__ void qpe(qreg q, QPEOracleSignature oracle) {
  ...
  // Using the Q# Kernel (wrapped as a QUKKOS kernel)
  QUKKOS__IQFT(counting_qubits);
