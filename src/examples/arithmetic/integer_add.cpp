...
int main(int argc, char **argv) {
  ...
  qukkos_expect(a.counts().size() == 2);
  qukkos_expect(a.counts()["111"] > 400);
  qukkos_expect(a.counts()["110"] > 400);
  ...
  b.print();
  qukkos_expect(b.counts().size() == 2);
  qukkos_expect(b.counts()["110"] > 400);
  qukkos_expect(b.counts()["010"] > 400);
  ...
  // |a*x + b> = |4> + |10>
  b_reg.print();
  qukkos_expect(b_reg.counts().size() == 2);
  // 4
  qukkos_expect(b_reg.counts()["00100"] > 400);
  // 10 = 8 + 2
  qukkos_expect(b_reg.counts()["01010"] > 400);
  x_reg.print();
  ...
  x_reg2.print();
  qukkos_expect(x_reg2.counts().size() == 2);
  // 2
  qukkos_expect(x_reg2.counts()["010"] > 400);
  // 6
  qukkos_expect(x_reg2.counts()["011"] > 400);
  return 0;
}