...
for run_id in range(n_runs): 
    ...
    os.system('/usr/local/aideqc/llvm/bin/clang++  -Wno-unused-command-line-argument -Wno-override-module -rdynamic -Wl,-rpath,/usr/local/aideqc/qukkos/lib:/usr/local/aideqc/qukkos/lib:/usr/local/aideqc/llvm/lib:/usr/local/aideqc/qukkos/clang-plugins -L /usr/local/aideqc/qukkos/lib -lqukkos -lqrt -lqukkos-jit -lqukkos-quasimo -L /usr/local/aideqc/qukkos/lib -lxacc -lCppMicroServices -lxacc-quantum-gate -lxacc-pauli -lxacc-fermion -lpthread -lqir-qrt -D__internal__qukkos__compile__backend="qpp" qir/trotter.o driver.cpp -o qir/a.out')
    end = time.time()
