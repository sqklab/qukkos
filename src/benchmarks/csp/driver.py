...
qukkosExe = str(pathlib.Path.home()) + "/.xacc/bin/qukkos"
...
# Notes:
# staq suite source: https://github.com/tnguyen-ornl/qukkos/tree/tnguyen/opt-data/benchmarks/resources/staq
# qasm suite source: https://github.com/tnguyen-ornl/qukkos/tree/tnguyen/opt-data/benchmarks/resources/qasm
# chemistry suite source: https://github.com/tnguyen-ornl/qukkos/tree/tnguyen/opt-data/benchmarks/resources/chemistry
def runBenchmarkSuite(suiteName):
  ...
  for srcFile in listOfSrcFiles:
    ...
    for passName in passesToRun:
      # Single pass data:
      compileProcess = subprocess.Popen([qukkosExe, "-DTEST_SOURCE_FILE=\"" + srcFile + "\"", "-o", testCaseName, "-opt-pass", passName, "-print-opt-stats", "circuit_opt_benchmark.cpp"],
    ...
    OPT_LEVEL = 1
    compileProcess = subprocess.Popen([qukkosExe, "-DTEST_SOURCE_FILE=\"" + srcFile + "\"", "-o", testCaseName, "-opt", str(OPT_LEVEL), "-opt-pass", "voqc", "-print-opt-stats", "circuit_opt_benchmark.cpp"],
                                      stdout=subprocess.PIPE,
                                      stderr=subprocess.PIPE)
    stdout, stderr = compileProcess.communicate()
    if len(stderr) > 1:
